...
mentee_data = st.secrets["mentee_data"].replace(
    '/edit?gid=', '/export?format=csv&gid=')
mentor_data = st.secrets["mentor_data"].replace(
    '/edit?gid=', '/export?format=csv&gid=')

mentees_table = pd.read_csv(mentee_data)
mentors_table = pd.read_csv(
    mentor_data, index_col="verification_code")
